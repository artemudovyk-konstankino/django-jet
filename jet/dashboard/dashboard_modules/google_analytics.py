...
from django.utils.encoding import force_str, force_str as force_unicode
...
class GoogleAnalyticsBase(DashboardModule):
	...
	def counter_attached(self):
		if self.credential is None:
			self.error = mark_safe(_('Please <a href="%s">attach Google account and choose Google Analytics counter</a> to start using widget') % reverse('jet-dashboard:update_module', kwargs={
				'pk': self.model.pk}))
			return False
		elif self.counter is None:
			self.error = mark_safe(_('Please <a href="%s">select Google Analytics counter</a> to start using widget') % reverse('jet-dashboard:update_module', kwargs={
				'pk': self.model.pk}))
			return False
		else:
			return True
		...
		if self.counter_attached():
			...
			try:
				...
				if isinstance(e, AccessTokenRefreshError):
					error += _(' Try to <a href="%s">revoke and grant access</a> again') % reverse('jet-dashboard:update_module', kwargs={
						'pk': self.model.pk})
